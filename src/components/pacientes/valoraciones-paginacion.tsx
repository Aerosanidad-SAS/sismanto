...
import { VALORACIONES_POR_PAGINA, valoracionesAQuery } from "@/lib/valoraciones-lista";

/** "Mostrando 1–25 de 240 · Página 1 de 10" con « ‹ 1 2 3 › », igual que la de Servicios. */
export function ValoracionesPaginacion({ q, pagina, total }: { q: string; pagina: number; total: number }) {
  const paginas = Math.max(1, Math.ceil(total / VALORACIONES_POR_PAGINA));
  const actual = Math.min(pagina, paginas);
  const desde = total === 0 ? 0 : (actual - 1) * VALORACIONES_POR_PAGINA + 1;
  const hasta = Math.min(actual * VALORACIONES_POR_PAGINA, total);
  ...
  const boton = (p: number, contenido: React.ReactNode, etiqueta: string, activo = false, deshabilitado = false) =>
    deshabilitado ? (
      ...
      <Link
        href={`/valoraciones${valoracionesAQuery(q, p)}`}
        aria-label={etiqueta}
  ...
  return (
    <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
      ...
      {paginas > 1 && (
        <nav aria-label="Páginas de valoraciones" className="flex flex-wrap gap-1">
