...
import { PACIENTES_POR_PAGINA, pacientesAQuery } from "@/lib/pacientes-lista";

/** "Mostrando 1–100 de 1.234 · Página 1 de 13" con « ‹ 1 2 3 › », igual que la de Servicios. */
export function PacientesPaginacion({ q, pagina, total }: { q: string; pagina: number; total: number }) {
  const paginas = Math.max(1, Math.ceil(total / PACIENTES_POR_PAGINA));
  const actual = Math.min(pagina, paginas);
  const desde = total === 0 ? 0 : (actual - 1) * PACIENTES_POR_PAGINA + 1;
  const hasta = Math.min(actual * PACIENTES_POR_PAGINA, total);
  ...
  const boton = (p: number, contenido: React.ReactNode, etiqueta: string, activo = false, deshabilitado = false) =>
    deshabilitado ? (
      ...
      <Link
        href={`/pacientes${pacientesAQuery(q, p)}`}
        aria-label={etiqueta}
  ...
  return (
    <div className="flex flex-col gap-2 sm:flex-row sm:items-center sm:justify-between">
      ...
      {paginas > 1 && (
        <nav aria-label="Páginas de pacientes" className="flex flex-wrap gap-1">
