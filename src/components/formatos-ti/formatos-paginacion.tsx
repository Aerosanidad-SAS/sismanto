...
import { FORMATOS_POR_PAGINA, aQuery } from "@/lib/formatos-ti/comun";

/** Paginación común de los Formatos TI: "Mostrando 1–25 de N · Página 1 de M" con « ‹ 1 2 3 › ». */
export function FormatosPaginacion({
  ruta,
  filtros,
  pagina,
  total,
}: {
  /** Ruta de la página, p. ej. "/formatos-ti/acta-entrega". */
  ruta: string;
  /** Filtros activos (q, tipo, sede…): se conservan al cambiar de página. */
  filtros: Record<string, string>;
  pagina: number;
  total: number;
}) {
  const paginas = Math.max(1, Math.ceil(total / FORMATOS_POR_PAGINA));
  const actual = Math.min(pagina, paginas);
  const desde = total === 0 ? 0 : (actual - 1) * FORMATOS_POR_PAGINA + 1;
  const hasta = Math.min(actual * FORMATOS_POR_PAGINA, total);
  ...
  const boton = (p: number, contenido: React.ReactNode, etiqueta: string, activo = false, deshabilitado = false) =>
    deshabilitado ? (
      ...
      <Link
        href={`${ruta}${aQuery({ ...filtros, pagina: p })}`}
        aria-label={etiqueta}
